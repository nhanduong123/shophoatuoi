<?php
	...
	if(isset($_POST["themnhacc"]))
	{
		$tenncc;$mancc;$ttncc;$sdt;
		...
		$query="select * from nhacungcap where nhacungcap.mancc=mancc";
		$data=$loai->queryXuatnhacc($query, $mancc);
		//print_r($data);
		
		function ktncc($mancc1){
			$loai = new db();
			$query="select * from nhacungcap where nhacungcap.mancc=mancc";
			$data=$loai->queryXuatnhacc($query, $mancc1);
			foreach($data as $key=>$value){
				if($value['Mancc']==$mancc1){
					return false;
				}
			...
			if(ktncc($mancc,$data)==false)
				...
				$query=	"insert into nhacungcap (mancc, tenncc, thongtin)
						values (:mancc,:tenncc,:thongtin)";
				$data=$loai->queryThemncc($query,$mancc,$tenncc,$thongtin);
				?>
				<script type="text/javascript">
...
<div class="container">
      ...
      <form class="form-dk" action="themnhacungcap.php" method="post">
        <h2 class="form-signin-heading">Thêm Nhà Cung Cấp</h2>
        <label for="tncc" class="sr-only">Tên nhà cung cấp</label>
        <input type="text" id="tncc" class="form-control" name="tenncc" placeholder="Tên nhà cung cấp" required autofocus>
        <label for="mancc" class="sr-only">Mã Nhà Cung Cấp</label>
        <input type="text" id="mncc" class="form-control" name="mancc" placeholder="Mã nhà cung cấp" required>
        <label for="tt" class="sr-only">Thông Tin</label>
        <input type="text" id="tt" class="form-control" name="thongtin" placeholder="Thông tin" required autofocus>
        <button class="btn btn-lg btn-primary btn-block" name="themnhacc" type="submit">Thêm Nhà Cung Cấp</button>
      </form>
