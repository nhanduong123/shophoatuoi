<?php
	...
	if(isset($_POST["xoathanhvien"]))
	{
		...
		if (isset($_POST["matv"]))
			$matv = $_POST["matv"];
		$query="select * from thanhvien where thanhvien.matv=matv";
		$data=$loai->queryXuatthanhvien($query, $matv);
		//print_r($data);
		
		function kttv($matv1){
			$loai = new db();
			$query="select * from thanhvien where thanhvien.matv=matv";
			$data=$loai->queryXuatthanhvien($query, $matv1);
			...
			if(kttv($matv,$data)==false)
				...
				$query=	"Delete From thanhvien Where matv like :matv";
				$data=$loai->queryXoatv($query,$matv);
				?>
				<script type="text/javascript">
...
<div class="container">
      ...
      <form class="form-dk" action="xoathanhvien.php" method="post">
        <h2 class="form-signin-heading">Xóa Thành Viên</h2>
        <label for="matv" class="sr-only">Mã Thành Viên</label>
        <input type="text" id="matv" class="form-control" name="matv" placeholder="Mã thành viên" required>
        <label for="lydo" class="sr-only">Lý do</label>
        <input type="text" id="lydo" class="form-control" placeholder="Lý do" required autofocus>
        <button class="btn btn-lg btn-primary btn-block" name="xoathanhvien" type="submit">Xóa Thành Viên</button>
      </form>
